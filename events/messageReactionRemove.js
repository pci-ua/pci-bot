exports.run = async(bot ,data) => {
    if(parseInt(data.message_id) == bot.ID_message_d_inscription) {
        ...
        if( index != -1 ) {
            membre.roles.remove( roles.find( x => x.name.replace(/ /ig,'') === bot.Accept_inscription[index]).id ); }
            ...
            if( index != -1 ) { membre.roles.remove( roles.find( x => x.name.replace(/ /ig,'') === bot.Accept_spe_master[index]).id );
        }
    }
}